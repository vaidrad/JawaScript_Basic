document.addEventListener('DOMContentLoaded', () =>{
    ...
    function filterProducts(product) {
        if (product.productName.indexOf(filterValue) !== -1) {
            return product;
        }
    }
    ...
    function renderProducts() {
        ...
        for(var val of product.filter(filterProducts)) {
            var div = document.createElement("div");
            div.className = "catalogProduct";
            div.innerHTML = '<img src="'+val.img+'" alt="catalogimage">'+"<h2>"+val.productName+"</h2><p>Цена товара: "+val.price+'</p>';
            var button = document.createElement("button");
            button.className = "buy";
            button.innerText = "Купить";
            button.setAttribute('article',val.article);
    ...
    input.addEventListener('input', (event) => { filterValue = event.target.value; renderProducts(); });
    document.getElementById('filter').prepend(input);
